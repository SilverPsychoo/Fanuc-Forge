import {CODES,CATEGORIES,SUPPORT_LABELS} from './gcode-data.js?v=4.0.1';
import {FanucInterpreter} from './interpreter.js?v=4.0.1';
import {StockSimulator} from './simulator.js?v=4.0.1';
...
const DEFAULT_WORKSPACE={units:'mm',x:220,y:120,z:30,resolution:3,renderQuality:'medium',zeroMode:'center',position:{x:0,y:0},table:{x:600,y:400,z:40,topZ:-30,slotDirection:'x'}};
const CUTTER_LIBRARY=[
  {id:'m-flat-3',system:'mm',type:'flat',name:'Fresa plana Ø3 mm',diameter:3,length:45},{id:'m-flat-4',system:'mm',type:'flat',name:'Fresa plana Ø4 mm',diameter:4,length:50},{id:'m-flat-6',system:'mm',type:'flat',name:'Fresa plana Ø6 mm',diameter:6,length:55},{id:'m-flat-8',system:'mm',type:'flat',name:'Fresa plana Ø8 mm',diameter:8,length:60},{id:'m-flat-10',system:'mm',type:'flat',name:'Fresa plana Ø10 mm',diameter:10,length:70},{id:'m-flat-12',system:'mm',type:'flat',name:'Fresa plana Ø12 mm',diameter:12,length:75},{id:'m-flat-16',system:'mm',type:'flat',name:'Fresa plana Ø16 mm',diameter:16,length:90},{id:'m-flat-20',system:'mm',type:'flat',name:'Fresa plana Ø20 mm',diameter:20,length:100},
  {id:'m-ball-4',system:'mm',type:'ball',name:'Punta bola Ø4 mm',diameter:4,length:50},{id:'m-ball-6',system:'mm',type:'ball',name:'Punta bola Ø6 mm',diameter:6,length:60},{id:'m-ball-8',system:'mm',type:'ball',name:'Punta bola Ø8 mm',diameter:8,length:70},{id:'m-ball-10',system:'mm',type:'ball',name:'Punta bola Ø10 mm',diameter:10,length:75},{id:'m-ball-12',system:'mm',type:'ball',name:'Punta bola Ø12 mm',diameter:12,length:80},
...
const config={stockTop:0,stockBounds:null,workspace:deepClone(DEFAULT_WORKSPACE),offsets:{},tools:{3:{diameter:10,length:70,type:'flat',name:'Fresa plana Ø10 mm',angle:90,displayUnit:'mm',presetId:'m-flat-10'}}};
...
const normalizeWorkspace=raw=>{
  ...
  return{units:source.units==='in'?'in':'mm',x:Number(source.x)||DEFAULT_WORKSPACE.x,y:Number(source.y)||DEFAULT_WORKSPACE.y,z,
    resolution:Number(source.resolution)||DEFAULT_WORKSPACE.resolution,renderQuality:['low','medium','high','ultra'].includes(source.renderQuality)?source.renderQuality:DEFAULT_WORKSPACE.renderQuality,zeroMode:source.zeroMode==='corner'?'corner':'center',
...
function stockSettings(){const u=$('#stockUnit').value;return normalizeWorkspace({units:u,x:lengthToMm($('#stockX').value,u),y:lengthToMm($('#stockY').value,u),z:lengthToMm($('#stockZ').value,u),resolution:+$('#resolution').value||3,renderQuality:$('#renderQuality').value||'medium',zeroMode:$('#zeroMode').value,position:{x:lengthToMm($('#stockPosX').value,u),y:lengthToMm($('#stockPosY').value,u)},table:{x:lengthToMm($('#tableX').value,u),y:lengthToMm($('#tableY').value,u),z:lengthToMm($('#tableZ').value,u),topZ:lengthToMm($('#tableTopZ').value,u),slotDirection:$('#tableSlotDirection').value}});}
...
function updateHud(state,step=null){
  ...
  $('#hudWcs').textContent=state.wcs||'G54';$('#hudMode').textContent=state.distance||'G90';$('#hudUnits').textContent=state.units==='G20'?'in':'mm';
...
function projectPayload(){return{format:'FANUC_FORGE_PROJECT',version:4.3,name:currentFileName,code:editor.value,config:deepClone(config),workspace:deepClone(config.workspace),camera:sim.getCamera(),displayMode:sim.viewMode,layout:{editorShare:getComputedStyle(document.documentElement).getPropertyValue('--editor-share').trim(),dockCollapsed:mainArea.classList.contains('dock-collapsed')}};}
...
function loadProjectObject(project){
  ...
  if(project.config){config.offsets=project.config.offsets||config.offsets;config.tools=project.config.tools||config.tools;}
...
function populateToolPresets(unit=toolDisplayUnit,selected='custom'){const list=CUTTER_LIBRARY.filter(p=>p.system===unit),groups={flat:'Fresas planas',ball:'Punta bola',face:'Planeado',drill:'Brocas',chamfer:'Chaflán / avellanado'};let html='<option value="custom">Personalizado</option>';for(const [type,label] of Object.entries(groups)){const items=list.filter(p=>p.type===type);if(items.length)html+=`<optgroup label="${label}">${items.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}</optgroup>`;}$('#toolPreset').innerHTML=html;$('#toolPreset').value=list.some(p=>p.id===selected)?selected:'custom';}
function updateToolPreview(){const number=Math.max(1,Math.trunc(+$('#toolNumber').value||3)),type=$('#toolType').value,name=$('#toolName').value.trim()||TOOL_TYPES[type],diameter=+$('#toolDiameter').value||0,u=$('#toolUnit').value;$('#toolTitle').textContent=`T${number} · ${name}`;$('#toolDescription').textContent=`${TOOL_TYPES[type]} · Ø${formatNumber(diameter)} ${unitLabel(u)} · modelo de corte ${type==='flat'||type==='face'?'plano':type==='ball'?'esférico':'angular'}.`;$('#toolIcon').className=`tool-icon ${type}`;$('#toolAngleRow').classList.toggle('hidden-field',!['drill','chamfer'].includes(type));$('#toolUnitMark').textContent=`(${unitLabel(u)})`;$('#toolLengthUnitMark').textContent=`(${unitLabel(u)})`;const preset=CUTTER_LIBRARY.find(p=>p.id===$('#toolPreset').value);$('#toolPresetInfo').textContent=preset?`${preset.name}. Diámetro real ${formatNumber(lengthInUnit(preset.diameter,u))} ${unitLabel(u)}.`:'Modo personalizado: puedes cambiar tipo, nombre, diámetro, longitud y ángulo.';}
function setToolForm(number,tool){tool={diameter:10,length:70,type:'flat',name:'Fresa plana Ø10 mm',angle:90,displayUnit:'mm',presetId:'custom',...tool};toolDisplayUnit=tool.displayUnit==='in'?'in':'mm';$('#toolNumber').value=number;$('#toolUnit').value=toolDisplayUnit;populateToolPresets(toolDisplayUnit,tool.presetId);$('#toolType').value=tool.type;$('#toolName').value=tool.name;$('#toolDiameter').value=lengthInUnit(tool.diameter,toolDisplayUnit);$('#toolLength').value=lengthInUnit(tool.length,toolDisplayUnit);$('#toolAngle').value=tool.angle||90;updateToolPreview();}
function loadToolFields(number=null){const ids=Object.keys(config.tools).map(Number).sort((a,b)=>a-b),toolNumber=number||ids[0]||3;setToolForm(toolNumber,config.tools[toolNumber]);}
function openToolDialog(){const current=interpreter?.state?.tool&&config.tools[interpreter.state.tool]?interpreter.state.tool:null;loadToolFields(current);$('#toolDialog').showModal();}
function applyTool(){const number=Math.max(1,Math.trunc(+$('#toolNumber').value||3)),u=$('#toolUnit').value,type=$('#toolType').value,diameter=Math.max(.001,lengthToMm($('#toolDiameter').value,u)),length=Math.max(0,lengthToMm($('#toolLength').value,u)),angle=Math.max(10,Math.min(180,+$('#toolAngle').value||90)),name=$('#toolName').value.trim()||`${TOOL_TYPES[type]} Ø${formatNumber($('#toolDiameter').value)} ${unitLabel(u)}`;config.tools[number]={diameter,length,type,name,angle,displayUnit:u,presetId:$('#toolPreset').value};sim.setCurrentTool(config.tools[number]);$('#toolDialog').close();updateSummaries();compileProgram({silent:true});toast(`Cortador T${number} guardado`);}
...
$('#showRapids').addEventListener('change',event=>{sim.showRapids=event.target.checked;sim.draw();});
...
$('#centerStockBtn').addEventListener('click',()=>{$('#stockPosX').value=0;$('#stockPosY').value=0;updatePiecePositionInfo();});
